...
```text
<версия Qwen Code, на которой проверен код>.<версия приложения>
```

Версия приложения начинается с `0` на каждой версии Qwen Code и увеличивается
на 1 при каждом следующем релизе приложения на той же версии Qwen Code.

Например, версия `0.24.0.1` означает, что код протестирован с Qwen Code
`0.24.0` и это второй релиз приложения на ней; `0.24.0.0` — первый релиз на
Qwen Code `0.24.0`; `0.25.0.0` — первый релиз, проверенный с Qwen Code
`0.25.0`.

Правила:

- Git tag repository, `version` в `../pyproject.toml` и `__version__` пакета —
  три совпадающих значения в формате выше;

- `qbs --version` показывает эту же версию;

- после обновления Qwen Code на новую версию `X` на ней прогоняются тесты и
  проверяется разбор артефактов Qwen Code (usage, chats, subagents,
  settings.json) и построение отчёта;

- если проверка на новой версии Qwen Code `X` успешна, все три значения
  устанавливаются равными `X.0` и создаётся соответствующий tag;

- если код не проходит проверку на новой версии Qwen Code, версия не
  повышается — сначала исправляются parsers;

- новый релиз приложения на уже проверенной версии Qwen Code `X` повышает
  версию с `X.N` до `X.N+1`.

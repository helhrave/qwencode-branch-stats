...
Версия инструмента всегда совпадает с версией Qwen Code, на которой код
проверен. Подробности — в разделах [Версионирование](#4-версионирование) и
...
После подключения hook из шага 1 эти файлы также автоматически
пересоздаются при каждом завершении Qwen session — отдельный запуск команды
...
Версия `qwencode-branch-stats` совпадает с версией Qwen Code, на которой код
проверен: например, версия `0.24.0` означает, что код протестирован с
Qwen Code `0.24.0` (`qwen --version`).

Совпадать должны три значения; четвёртое выводится из них:

| Место                                                    | Пример значения                     |
|----------------------------------------------------------|-------------------------------------|
| git tag репозитория                                       | `0.24.0`                            |
| `version` в `[project]` внутри `pyproject.toml`          | `0.24.0`                            |
| `__version__` в `src/qwencode_branch_stats/__init__.py` | `0.24.0`                            |
| вывод `qbs --version`                                    | `qwencode-branch-stats 0.24.0`      |

Текущая установленная версия показывает, с каким Qwen Code инструмент
проверен, а `qbs doctor` помогает быстро проверить совместимость с окружением.
...
3. Если проверка прошла, поднимите версию до версии Qwen Code в трёх
   местах (git tag, `pyproject.toml`, `__version__`) и переустановите
   пакет:
